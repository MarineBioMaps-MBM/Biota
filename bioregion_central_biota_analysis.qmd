...
centralcal_biota <- biota_clean %>%
  filter(pmep_region == 3) # Our regions are numbered 1-4, north to south
```
...
centralcal_biota <- st_transform(centralcal_biota, crs= 32610)
mpa <- st_transform(mpa, crs= 32610)
```

```{r}
# Join the data sets together
central_biota_mpa <- st_intersection(centralcal_biota, mpa)
```

### Calculate percentages of each biota type
```{r}
# Calculate area for each data set
central_biota_mpa$mpa_area_m2 <- st_area(central_biota_mpa)

# Create a sum of area column in mpa_transform
central_biota_mpa3 <- central_biota_mpa %>%
...
# Visualize results
ggplot(central_biota_mpa3, aes(x = cmecs_bc_category, y = pct_biota, fill = cmecs_bc_category)) +
  geom_col() +
