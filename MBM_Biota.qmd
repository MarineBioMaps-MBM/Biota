...
ggplot(mpa_group, aes(x = CMECS_BC_Category, y = pct_mpa_biota, fill= CMECS_BC_Category)) +
  ...
  labs(title = "Marine Biota Type by Area(km2) in Point Lobos SMR",
       ...
       y = "Percentage") +
  theme_bw() +
    scale_fill_manual(values = c('seagreen3','green4' )) +
...
```{r}
# Visualize results
ggplot(mpa_group_sub, aes(x = group, y = pct_substrate, fill= group)) +
  geom_col() +
  labs(title = "Marine Substrate Type by Area(km2) in Point Lobos SMR",
       x = "Substrate Category",
       y = "Percentage") +
  theme_bw() +
    scale_fill_manual(values = c('peachpuff4','beige' )) +
