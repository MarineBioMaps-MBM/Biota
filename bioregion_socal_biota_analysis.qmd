...
socal_biota <- biota_clean %>%
  filter(pmep_region == 4) # Our regions are numbered 1-4, north to south
```
...
socal_biota <- st_transform(socal_biota, crs= 32610)
mpa <- st_transform(mpa, crs= 32610)
```

```{r}
# Join the data sets together
socal_biota_mpa <- st_intersection(socal_biota, mpa)
```

### Calculate percentages of each biota type
```{r}
# Calculate area for each data set
socal_biota_mpa$mpa_area_m2 <- st_area(socal_biota_mpa)

# Create a sum of area column in mpa_transform
socal_biota_mpa4 <- socal_biota_mpa %>%
...
### Bar Graph of Biota Types within SoCal MPAs
```{r}
# Find percentage of each biota in the MPA
# Visualize results
ggplot(socal_biota_mpa4, aes(x = cmecs_bc_category, y = pct_biota, fill = cmecs_bc_category)) +
  geom_col() +
  labs(title = "Biota type by Area(m2) in SoCal",
       x = "Biota Group",
       y = "Percentage") +
